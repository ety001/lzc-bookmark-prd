...
**版本变更：** v3.0 → v4.0  
**变更摘要：** 明确版本号粒度（每变更 +1）、精确增量拉取谓词、补充部分冲突处理规则、细化新设备拉取步骤、缩小创建冲突合并范围、明确 index_weight 归一化行为  
...
| 数据模型 | 预留 `tenant_id` 字段（v1 恒为空字符串，v2 团队功能启用） | 未来扩展性 |
...
GlobalState {
  ...
  change_log: Change[]        // 变更记录（保留最近 1000 条或最近 7 天）
...
变更日志：
  - Change A: from_version=119, to_version=120  ← 不返回（已包含在 N 内）
...
| 创建冲突 | 同一 `parent_guid` + 同名 + 同 URL 的书签（无 GUID） | **保留两个书签，各自分配不同 GUID**（v4 变更） |

**v4 变更说明：** 创建冲突不再自动合并。两个设备创建同名同 URL 书签时：
- 如果 `parent_guid` 相同：视为重复，合并为同一 GUID
- 如果 `parent_guid` 不同：保留两个书签，各自分配不同 GUID（用户可能有意放在不同文件夹）
...
客户端行为：
1. 更新 last_known_version = 128（与 new_version 对齐）
2. 从离线队列移除已应用的 8 条变更
3. 保留 2 条冲突变更，标记为"待解决"
4. 下次推送时，仅推送冲突变更 + 新增变更（不重复推送已应用变更）
```
...
- 归一化行为：
  ...
  - **生成 `reindex` 类型变更记录**（特殊类型，客户端可忽略显示）
  - `reindex` 变更不计入变更日志上限（不占用 1000 条配额）
- 影响范围：仅当前文件夹下的节点
...
- 物理删除条件：`is_deleted=true` AND `deleted_at > 90 days` AND **无快照引用**

#### 5.1.3 同步逻辑
- 增量同步：基于版本号的变更日志（每条变更 +1 版本）
- 冲突检测：基于 GUID 和服务端接收时间（非客户端时间）
...
- 降级策略（Service Worker 休眠）：
  ...
  - 每次 alarms 触发或插件启动时，对比当前树与上次同步快照
  - **简化实现：唤醒时直接 GET /bookmarks?since_version=N 拉取服务端变更，不做本地 diff**
...
```json
{
  ...
  "data": {
    "bookmarks": {
      ...
      "title": "",
      "children": [
        { "guid": "...", "title": "...", "url": "...", "parent_guid": "root", "index_weight": 1.0 }
      ]
    },
    "is_baseline": true,
    ...
    "changes": [
      {
        ...
        "from_version": 120,
        "to_version": 121,
        "data": { "title": "New Bookmark", "url": "https://...", "parent_guid": "root", "index_weight": 1.0 }
      }
    ],
    "has_more": false
  },
  ...
  "changes": [
    {
      ...
      "guid": null,
      "local_id": "123",
      "data": { "title": "New Bookmark", "url": "https://...", "parent_guid": "root" }
    },
    {
...
**特殊变更类型：**
- `reindex`: index_weight 归一化生成，不计入变更日志上限
...
3. POST /sync，base_version=0，changes 包含所有节点（type=create）
   - 可分批提交（每批 500 条变更）
4. 服务端为每个节点生成 GUID
5. 服务端返回 generated_mappings: [{local_id, guid}]
6. 插件存储映射表到 storage.local
7. 后续同步使用 GUID 标识书签
```
...
4. 插件按 BFS 顺序创建本地书签：
   a. 创建根文件夹（Chrome 自动有 3 个根：书签栏、其他书签、移动设备）
   b. 遍历规范树，对每个节点：
      - 调用 chrome.bookmarks.create({parentId, title, url})
      - Chrome 返回新节点的 local_id
      - 建立映射：{local_id ↔ guid}
   c. 存储映射表到 storage.local
5. 后续增量同步使用 GUID
```
...
| 删除恢复 | 从历史版本回滚，重新创建书签 |
| 物理删除 | 软删除 90 天后，且无历史版本引用，执行物理删除 |
...
| 网络完全不可用 | 离线队列持久化，上限 5000 条，超限后合并变更 |
...
- API Key 分只读/读写两种权限
- 设备级权限撤销（立即生效）
- 操作审计日志：记录 API Key 哈希、设备 ID、操作类型、时间戳、IP（可选）
...
**关键变更:** 明确版本号粒度（每条变更 +1）、精确增量拉取谓词、补充部分冲突处理规则、细化新设备拉取 BFS 步骤、缩小创建冲突合并范围（同 parent_guid 才合并）、明确 index_weight 归一化生成 reindex 变更、本地备份默认关闭、只读 API Key 禁止写操作