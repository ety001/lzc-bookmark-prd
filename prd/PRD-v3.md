...
**版本变更：** v2.0 → v3.0  
**变更摘要：** 修正服务端数据模型（移除 local_id）、简化版本号机制、明确同步顺序、统一 API 格式、延长软删除周期、补充边界条件  
...
| 数据模型 | 预留 `tenant_id` 字段（v1 恒为空，v2 团队功能启用） | 未来扩展性 |
...
4. POST /api/v1/bookmarks/sync（推送本地变更）
       │
       ├── 返回 conflicts ──► 提示用户解决，暂停拉取
       │
       ▼
5. GET /api/v1/bookmarks?since_version=N（拉取远程变更）
       │
       ├── 返回 410 GONE ──► 全量重拉（变更日志溢出）
       │
       ▼
6. 应用远程变更到本地书签树
       │
       ▼
7. 更新本地 last_known_version
       │
       ▼
8. 同步完成
```
...
Bookmark {
  ...
  index_weight: float    // 排序权重（避免 index 冲突，见 §4.4）
...
1. **首次同步（全量上传）**：
   ...
   - 服务端为每个节点生成 GUID，返回 `id_mappings: [{local_id, guid}]`
...
Snapshot {
  version: integer
  bookmark_tree: Bookmark[]   // 完整规范树（序列化）
  created_at: integer
...
2. 服务端返回 `change_log` 中 `from_version >= last_known_version` 的所有变更
3. 插件应用变更，更新本地映射表
4. 插件提交本地变更（包含 `changes` 数组）
5. 服务端检测冲突，无冲突则追加到 `change_log`，版本号 +1

**变更日志溢出策略：**
- 保留条件：`COUNT(change_log) <= 1000` AND `age <= 7 days`
- 溢出行为：
  - 保留一个基准快照（`is_baseline=true`）
  - 新设备或落后设备（`last_known_version < baseline_version`）走全量同步
  - 全量同步返回完整规范树 + 新的 `id_mappings`（空，客户端自建）
...
| 创建冲突 | 两个设备创建同名同 URL 书签（无 GUID） | URL 去重，合并为同一 GUID |
...
**解决方案：使用 float 权重代替绝对 index**

```typescript
// 服务端
Bookmark {
  parent_guid: string
  index_weight: float  // 排序权重
}
```
...
- 本地备份：每次同步前自动导出书签 JSON 到 chrome.downloads（可选，默认开启）
...
- `client_request_id`: 幂等 ID，重复提交时返回相同结果（不重复应用变更）
...
**说明：** 限流维度为 **API Key + Device** 组合，同一 API Key 下不同设备独立限流。
...
```typescript
{
  ...
  index_weight: float    // 排序权重（非绝对 index）
  date_created: integer  // 服务端时间戳（Unix 毫秒）
  date_modified: integer // 服务端时间戳
  is_deleted: boolean    // 软删除标记
...
2. POST /sync，base_version=0，changes 包含所有节点（type=create）
3. 服务端为每个节点生成 GUID
4. 服务端返回 generated_mappings: [{local_id, guid}]
5. 插件存储映射表到 storage.local
6. 后续同步使用 GUID 标识书签
```
...
4. 插件为每个节点生成/分配 local_id
5. 插件建立本地映射表：{local_id ↔ guid}
6. 后续增量同步使用 GUID
```
...
| 部分变更已应用但响应丢失 | 重试时服务端检测 client_request_id，返回上次结果 |
...
| Manifest V3 限制 | Service Worker 5 分钟休眠 | 使用 chrome.alarms API，休眠期间通过全量 diff 检测变更 |
| 数据丢失 | 同步错误导致书签被覆盖 | 同步前自动本地备份（导出 JSON 到 chrome.downloads） |
...
**关键变更:** 移除服务端 local_id、简化版本号机制、明确先推后拉顺序、统一 API 格式、延长软删除至 90 天、补充 410 溢出处理