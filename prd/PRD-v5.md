...
**版本变更：** v4.0 → v5.0  
**变更摘要：** 修正变更日志截断逻辑、补充同批 create 的 GUID 分配协议、明确 Chrome 三根映射策略、完善冲突检测时间窗口、补充 reindex 变更格式、增加 limit 分页参数  
**评审来源：** GLM-5、Composer 2、Kimi-k2.5、Kimi Code 四方审计汇总
...
Bookmark {
  ...
  title: string
  url: string | null
  parent_guid: string    // 父节点的 guid（"root" 表示根目录）
...
Snapshot {
  ...
  bookmark_tree: Bookmark[]   // 完整规范树（JSON 序列化，gzip 压缩）
  created_at: integer
...
**变更日志溢出策略：**
- 溢出条件：截断后，`baseline_version` 与 `current_version` 差距过大
- 溢出行为：
  - 新设备或落后设备（`last_known_version < baseline_version`）走全量同步
  - 服务端返回 410 GONE + E008 错误码
  - 全量同步返回完整规范树 + `is_baseline=true`
...
| 类型 | 检测条件 | 解决策略 |
|------|----------|----------|
| 更新冲突 | 同一 GUID，两个变更的 `from_version` 相同 | 用户选择 / 最新优先 |
| 删除冲突 | 设备 A 删除，设备 B 修改同一 GUID | 默认保留修改（防误删） |
| 移动冲突 | 同一 GUID，两个变更的 `parent_guid` 不同 | 用户选择 |
| 重命名冲突 | 同一 GUID，两个变更的 `title` 不同 | 最新优先（服务端接收时间） |
| 创建冲突 | 同一 `parent_guid` + 同名 + 同 URL 的书签（无 GUID） | **仅同文件夹内合并**（v5 明确） |

**创建冲突策略（v5 明确）：**
```
场景 1：设备 A 和设备 B 都在"书签栏/工作"文件夹下创建了同名同 URL 的书签
  → 视为重复，合并为同一 GUID

场景 2：设备 A 在"书签栏/工作"创建，设备 B 在"其他书签/收藏"创建同名同 URL 书签
  → 保留两个书签，各自分配不同 GUID（用户可能有意放在不同文件夹）

场景 3：用户在同一文件夹下故意创建两条完全相同的快捷方式（罕见但合法）
  → v1 简化策略：仍合并为一条。如需要两条，用户可手动复制。
```
...
3. 从离线队列移除已应用的 8 条变更
4. 保留 2 条冲突变更，标记为"待解决"
5. 下次推送时：
   - 冲突变更的 base_version 更新为 new_version（128）
   - 仅推送冲突变更 + 新增变更（不重复推送已应用变更）
```
...
2. 解析规范树第一层：
   - guid="root_bar" → 映射到 Chrome id="1"
   - guid="root_other" → 映射到 Chrome id="2"
   - guid="root_mobile" → 映射到 Chrome id="3"
3. 按 BFS 顺序创建书签：
   - 不创建第一层（Chrome 已有固定根）
   - 从第二层开始调用 chrome.bookmarks.create({parentId: "1", ...})
   - Chrome 返回每个节点的 local_id
   - 建立映射：{local_id ↔ guid}
...
- 物理删除条件（v5 明确）：`is_deleted=true` AND `deleted_at > 90 days` AND **最近 30 天快照未引用**

#### 5.1.3 同步逻辑
- 增量同步：基于版本号的变更日志（每条变更 +1 版本）
- 冲突检测：基于 GUID 和 from_version（时间窗口仅用于 UI 提示）
- 冲突解决：支持三种策略（use_local / use_remote / merge）
- 溢出处理：变更日志溢出时返回 410 GONE，触发客户端全量重拉
...
- 全量重拉：收到 410 GONE 时，清空本地映射表，重新全量拉取
...
| E003 | 409 | 版本冲突（from_version 严重落后，需全量重拉） |
| E004 | 400 | 设备未注册（仅显式调用设备 API 时） |
| E005 | 400 | 数据格式错误 |
| E006 | 429 | 请求频率超限 |
| E007 | 500 | 服务端错误 |
| E008 | 410 | 变更日志溢出，需全量同步 |

**409 E003 vs 200 + conflicts 边界（v5 明确）：**
```
返回 409 E003 的场景：
...
```json
{
  ...
  "data": {
    "bookmarks": {
      ...
      "title": "",
      "children": [
        { "guid": "root_bar", "title": "书签栏", "parent_guid": "root", "index_weight": 1.0 },
        { "guid": "root_other", "title": "其他书签", "parent_guid": "root", "index_weight": 2.0 },
        { "guid": "root_mobile", "title": "移动设备书签", "parent_guid": "root", "index_weight": 3.0 }
      ]
    },
    "is_baseline": true,
  ...
  "changes": [
    {
      ...
      "guid": null,
      "local_id": "123",
      "parent_local_id": "100",  // 父节点的 local_id（同批创建时使用）
...
服务端处理：
1. 处理第一条：生成 guid_A，缓存 { "100": "guid_A" }
2. 处理第二条：从缓存查找 "100" → "guid_A"，设置 parent_guid="guid_A"
```
...
重试请求：client_request_id="req_001", base_version=120（相同）
  → 不应用变更，返回 version=128（可能有其他设备提交了变更）
```
...
4. POST /sync，base_version=0，changes 包含所有节点（type=create）
   - 第一层节点映射到服务端 root_bar、root_other、root_mobile
   - 可分批提交（每批 500 条变更）
5. 服务端为每个节点生成 GUID
   - 按顺序处理，解析 parent_local_id 为正式 GUID
6. 服务端返回 generated_mappings: [{local_id, guid}]
7. 插件存储映射表到 storage.local
8. 后续同步使用 GUID 标识书签
...
5. 按 BFS 顺序创建本地书签（从第二层开始）：
   a. 遍历规范树第二层节点
   b. 对每个节点调用 chrome.bookmarks.create({parentId: "1", title, url})
...
| 版本重叠 | 两个变更的 `from_version` 相同 |
| 时间窗口 | 服务端接收时间相差 < 10 秒（仅用于 UI 提示"并发修改"） |

**注意：** 时间窗口不影响冲突判定，仅用于 UI 提示。即使时间相差 > 10 秒，只要 from_version 相同，仍判定为冲突。
...
| 删除恢复 | 从历史版本回滚，重新创建书签 |
| 物理删除 | 软删除 90 天后，且最近 30 天快照未引用，执行物理删除 |
...
| 部分变更已应用但响应丢失 | 重试时服务端检测 client_request_id，返回当前最新状态 |
| GET /bookmarks 返回 410 | 清空本地映射表，全量重拉 |
| 网络完全不可用 | 离线队列持久化，上限 5000 条，超限后合并变更（仅合并未提交过的） |
...
- [ ] 设备管理功能
- [ ] 多设备同步测试（2-3 设备）
- [ ] 边界测试（1000+ 书签、弱网、离线、410 溢出、部分冲突）
- [ ] 文档编写（用户指南、API 文档）
- [ ] 自动化测试用例编写
...
| Manifest V3 限制 | Service Worker 5 分钟休眠 | 使用 chrome.alarms API，唤醒时直接拉取服务端变更 |
| 数据丢失 | 同步错误导致书签被覆盖 | 本地备份（可选，默认关闭），导出 JSON 到 chrome.downloads |
| 变更日志溢出 | 长期离线设备无法增量同步 | 返回 410 GONE，触发全量重拉 |
...
**创建时间:** 2026-04-16  
**状态:** 已评审，待开发  
**评审者:** GLM-5, Composer 2, Kimi-k2.5, Kimi Code  
**关键变更:** 修正变更日志截断逻辑（OR/AND 统一）、补充同批 create 的 GUID 分配协议、明确 Chrome 三根映射策略、完善冲突检测时间窗口语义、补充 reindex 变更格式、增加 limit 分页参数、明确只读 API Key 权限矩阵